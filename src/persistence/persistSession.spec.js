...
test('One new tab creates one new bookmark', async t => {
	...
	chrome.bookmarks.get.withArgs(bookmarkFolderId).resolves({
		id: bookmarkFolderId,
		children: [
			{
				index: 1,
	...
	t.assert(chrome.bookmarks.create.calledOnceWith({index: 2, url: githubDotCom}))
})
...
test('Two new tabs creates two new bookmarks', async t => {
	...
	t.assert(chrome.bookmarks.create.calledWith({index: 2, url: githubDotCom}))	
	t.assert(chrome.bookmarks.create.calledWith({index: 3, url: playwrightDotCom}))
})
...
test('One tab moved moves one bookmark', async t => {
	...
	t.assert(chrome.bookmarks.move.calledWith('2', {index: 3}))	
	t.assert(chrome.bookmarks.move.calledWith('3', {index: 2}))
})