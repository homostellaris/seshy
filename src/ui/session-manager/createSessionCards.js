...
function createUnsavedSessionCards (windows) {
	windows.forEach(window => {
		...
		createSessionCard(session, currentlyOpenSessionListId)
	})
}
...
async function createShelvedSessionCards () {
	...
	shelvedBookmarkFolders.forEach(async bookmarkFolder => {
		...
		createSessionCard(session, shelvedSessionListId)
	})
}

function createSessionCard (session, listId) {
	...
	sessionCard.addEventListener('focus', (event) => {
