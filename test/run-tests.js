...
driver.findElement(By.css('.jasmine-spec-detail.jasmine-failed')).then(
  failuresContainer => {
    fail()
  },
  e => {
...
function fail () {
  driver
    .findElement(By.className('jasmine-failures'))
    .then(jasmineFailures => {
      console.log(
        'The jasmine-failures element was located. Now expecting it to have child divs for the failed ' +
          'specs. Trying to find...'
      )
      return jasmineFailures.findElements(
        By.css('.jasmine-failures .jasmine-description a:last-child')
      )
    })
...
function exitCodeOne (jasmineFailedSpecs) {
  ...
  promise
    .map(jasmineFailedSpecs, e => e.getText())
    .then(texts => {
      console.error(texts)
      process.exit(1)
    })
